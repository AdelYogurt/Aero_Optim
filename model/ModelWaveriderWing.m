classdef ModelWaveriderWing < handle
    ...
    methods
        ...
        function self=ModelWaveriderWing(partitions,geo_param,mesh_param,CFD_param,...
                dir_temp,REMOVE_TEMP,run_description,out_logger)
            % WW model setup function
            %
            % notice:
            ...
            if ~isempty(mesh_param)
                switch mesh_param.solver
                    case 'SU2_DEF'
                        ...
                        if ~isfield(mesh_param,'dat_filestr'), mesh_param.dat_filestr='model/WW.dat';end
                        if ~isfield(mesh_param,'mesh_filestr'), mesh_param.mesh_filestr='model/WW.su2';end
                    otherwise
        ...
        % solve waverider wing aerodynamic model function
        ...
        function geo_out=geoModule(self,geo_in)
            % calculate geo out
            %
            WW=WaveriderWing(geo_in.total_length,geo_in.param,geo_in.type);
            geo_out.mesh_point=WW.calMeshPoint(self.geo_param.mesh_coord);

            mesh_data=WW.getWGSMesh();
