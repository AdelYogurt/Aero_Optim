...
idx_base=1;
idx_zone=1;
cgns_filename='..\HGV\SU2\WWDB.cgns';
marker_name_list={'WAVERIDER','WING_FRONT','TRI_WING_FRONT','HEAD_SIDE',...
    'WING_SIDE','WING','TRI_WING'};

% open file
[idx_file, ierr] = cg_open(cgns_filename, CG_MODE_READ);

% read point
[basename, icelldim, iphysdim, ierr] = cg_base_read(idx_file, idx_base);
[zonename, size, ierr] = cg_zone_read(idx_file, idx_base, idx_zone);
...
for idx_BC=1:out_nbocos
    ...
    io_NormalList=0;
    [out_boconame, out_bocotype, out_ptset_type, out_npnts, io_Normalidx, out_NormalListSize, out_NormalDataType, out_ndataset, ierr] =...
...
save('mesh_data.mat','point_list','marker_index_list')

function [boolean,idx]=judgeBCName(name,BC_name_list)
boolean=false(1);
for idx=1:length(BC_name_list)
    if strcmp(['BC1_on_',BC_name_list{idx}],name)
        boolean=true(1);
        return;
    end
end
