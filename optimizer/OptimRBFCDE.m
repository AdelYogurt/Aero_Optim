classdef OptimRBFCDE < handle
    ...
    % main function
    methods
        function self=OptimRBFCDE(NFE_max,iter_max,obj_torl,con_torl)
    ...
    methods
        function x_infill=searchGlobal(self,X,Obj,Con,Coneq,Vio,...
            ...
            [obj_pred_DE_list]=self.obj_fcn_srgt(X_DE);

            % variance were replaced by x distance to exist x
            % distance scale
            dis=zeros(size(X_DE,1),size(X,1));
            for vari_idx=1:vari_num
                dis=dis+(X_DE(:,vari_idx)-X(:,vari_idx)').^2;
            end
            D=min(sqrt(dis),[],2);
            ...
            if self.FLAG_CON
                if ~isempty(self.con_fcn_srgt)
                    [con_pred_DE_list,coneq_pred_DE_list]=self.con_fcn_srgt(X_DE);
                    con_var_DE_list=obj_var_DE_list;
                    coneq_var_DE_list=obj_var_DE_list;
                end

                vio_DE_list=[];
                if ~isempty(con_pred_DE_list),vio_DE_list=[vio_DE_list,max(max(con_pred_DE_list-self.con_torl,0),[],2)];end
                if ~isempty(coneq_pred_DE_list),vio_DE_list=[vio_DE_list,max(abs(coneq_pred_DE_list-self.con_torl),[],2)];end
                vio_DE_list=max(vio_DE_list,[],2);
            ...
            function X_new=DERand(low_bou,up_bou,X,F,x_num,rand_num)
                [x_num__,vari_num__]=size(X);
                X_new=zeros(x_num,vari_num__);
                for x_idx__=1:x_num
                    idx__=randi(x_num__,2*rand_num+1,1);
            ...
            function X_new=DECurrentRand(low_bou,up_bou,X,F)
                [x_num__,vari_num__]=size(X);
                X_new=zeros(x_num__,vari_num__);
                for x_idx__=1:x_num__
                    idx__=randi(x_num__,3,1);
            ...
            function X_new=DECrossover(low_bou,up_bou,X,V,C_R)
                [x_num__,vari_num__]=size(X);
                X_new=X;
                rand_num=rand(x_num__,vari_num__);
                Bool_idx__=rand_num < C_R;
                X_new(Bool_idx__)=V(Bool_idx__);
                for x_idx__=1:x_num__
                    X_new(x_idx__,:)=max(X_new(x_idx__,:),low_bou);
                    X_new(x_idx__,:)=min(X_new(x_idx__,:),up_bou);
                end
        ...
        function x_infill=searchLocal(self,X,Obj,Con,Coneq,Vio,...
            ...
            [x_infill,obj_infill,exit_flag,output_fmincon]=fmincon(obj_fcn_local,x_init,[],[],[],[],...
    ...
    methods(Static)
        function [obj_fcn_srgt,con_fcn_srgt,Srgt_obj,Srgt_con,Srgt_coneq]=getSrgtFcnRBF...
            ...
            for obj_idx=1:size(obj_list,2)
                Srgt_obj{obj_idx}=surrogateRBF(x_list,obj_list(:,obj_idx));
            end
            ...
            if ~isempty(con_list)
                ...
                for con_idx=1:size(con_list,2)
                    Srgt_con{con_idx}=surrogateRBF(x_list,con_list(:,con_idx));
                end
            else
                Srgt_con=[];
            end
            ...
            if ~isempty(coneq_list)
                ...
                for coneq_idx=1:size(coneq_list,2)
                    Srgt_coneq{coneq_idx}=surrogateRBF(x_list,coneq_list(:,coneq_idx));
                end
            else
                Srgt_coneq=[];
            end
...
function model_RBF=surrogateRBF(X,Y,basis_fcn)
...
% basis_fcn(optional, default is r.^3)
%
% output:
% model_RBF(a radial basis function model)
%
% abbreviation:
...
model_RBF.X=X;
model_RBF.Y=Y;
model_RBF.x_num=x_num;
model_RBF.vari_num=vari_num;
...
model_RBF.RBF_matrix=RBF_matrix;
model_RBF.beta=beta;

model_RBF.predict=pred_fcn;
model_RBF.Rsq=@() calTrainRsqRBF(model_RBF);

    function [beta,RBF_matrix]=calRBF...
    ...
    function [Rsq,Error_pred]=calTrainRsqRBF(model_RBF)
        ...
        %
        inv_RBF_matrix=model_RBF.RBF_matrix\eye(model_RBF.x_num);
        Error_pred=model_RBF.beta*model_RBF.stdD_Y./diag(inv_RBF_matrix);
        Rsq=1-sum(Error_pred.^2)/((model_RBF.stdD_Y)^2*(model_RBF.x_num-1));
    end
